...
/*
 * Arguments (attached to the action instance by rrel_1..rrel_4):
 ...
 *   subject via nrel_prev_attempt; sets the attempt as the action result.
 */
ScResult RecordAttemptAgent::DoProgram(ScAction & action)
{
  auto const & [subject, act, object, outcome] = action.GetArguments<4>();
