...
Финальный набор (проверен по логу: `MCP loading complete: 5/5 server(s) ready, 78 tool(s)`):

| Сервер | Инструментов | Что даёт |
|---|---|---|
| `oneiro` | 5 | память графа и вопрос человеку с кнопками |
| `memora` | 42 | личная память владельца; там уже есть **гибридный поиск** (`memory_hybrid_search`), слияние, свёртка, дубликаты, кластеры |
| `knowledge-rag` | 5 | поиск по базе знаний воркспейса |
| `searchmcp` | 1 | поиск в сети |
| `playwright` | 25 | браузер, который действует, а не только читает |

Не включены, и почему:

- `chroma`: `uvx` тянул torch, scipy и ещё десяток пакетов (200+ МБ) и не успел
  поздороваться за 120 секунд; `memora` уже закрывает векторный поиск.
- `sqlite-mcp`, `obsidian`, `sequential-thinking`: падали при старте и без нашего
  участия; их можно вернуть по одному, когда понадобятся.
- `telegram-mcp-bot`, `telegram-mcp-userbot` и всё, что умеет отправлять наружу:
  по решению владельца отправка идёт только после нажатия кнопки, значит сначала
  в них надо встроить наш канал согласования, а не давать агенту прямой доступ.
- `unity` (мёртвый адрес), `photoshop`, `blender`, `docker` и розыскные серверы:
  к работе помощника по документам отношения не имеют.

Урок, который стоил трёх отказов: **хосту нельзя доверять PATH**. Все серверы на
`npx` падали с `WinError 2` («не удаётся найти указанный файл»), хотя `npx` в
системе есть. Теперь `host-wire.py` подставляет абсолютные пути в команды и
прописывает каталог в `PATH` записи.
...
- Хост поднимает пять серверов сам и перечисляет 78 инструментов; при повторных
  запусках их количество не менялось.
