...
- [ ] **Step 1: Start Docker Desktop** and wait for the Linux engine (blocked if it does not come up — fix first, everything else waits).
- [ ] **Step 2: Bring up ostis-example-app:** `docker compose up --build -d` in `projects/ostis/ostis-example-app`, wait for `machine` healthcheck to pass.
- [ ] **Step 3: Verify sc-web** serves on `http://localhost:8000`.
- [ ] **Step 4: py-sc-client smoke** — connect to `localhost:8090`, resolve one known sc-element from the example KB.
- [ ] **Step 5: Commit** nothing (env stage); record result in docs/PROGRESS.md.
...
- [ ] **Step 1: Copy build skeleton** from ostis-example-app (CMakeLists, docker-compose, repo.path, interface/) into `oneiro-ostis/`.
- [ ] **Step 2: Write failing test** `test_record_then_retrieve_roundtrip` (writes one attempt with subject+time link, retrieves, asserts equality).
- [ ] **Step 3: Run test — expect FAIL** (agents/ontology not implemented yet): `pytest tests/test_core_loop.py -v`.
- [ ] **Step 4: Author experience ontology** in `.scs`; rebuild KB via sc-builder inside the image.
- [ ] **Step 5: Implement RecordAttempt + RetrieveAttempt** C++ agents on ScActionInitiatedAgent; wire action classes.
- [ ] **Step 6: Implement bridge.py** so the test drives the real stack.
- [ ] **Step 7: Run test — expect PASS;** commit `feat: core experience loop`.
...
- [ ] **Step 1: Deterministic world + test** (same seed → identical trace hash, twice).
- [ ] **Step 2: Baseline agents** (context-only; flat dict store) + score().
- [ ] **Step 3: OSTIS-memory agent** reusing Stage 2 API; episodes linked with temporal + provenance relations.
- [ ] **Step 4: Ablations:** memory without temporal links; without provenance (config flags on the ontology side).
- [ ] **Step 5: Results table script:** `python -m metrics.build_table > docs/results.md` reproduces all numbers.
- [ ] **Step 6: Run full suite; commit `feat: scientific layer (baselines, metrics, ablations)`.**
...
- [ ] **Step 1: Replay exactness test** — replayed outcomes == recorded outcomes on the full tree (property: dreaming never executes).
- [ ] **Step 2: Replay engine** over alternative strategy descriptors (branch choice, parallel grouping, stopping rule).
- [ ] **Step 3: LLM candidate generation** (N strategies per dream; JSON policy descriptors; strict schema).
- [ ] **Step 4: Dream test** — at least one dream run whose deployed winner improves next online round score vs previous strategy.
- [ ] **Step 5: Commit `feat: dream cycle (replay judge + LLM candidates + deploy)`.**
...
- [ ] **Step 1: Two-round test** — round 2 score ≥ round 1 with dream deployed (or honest zero-result documented).
- [ ] **Step 2: Round dynamics script** — curves built from real logs into docs/.
- [ ] **Step 3: Commit `feat: recursive self-improvement loop`.**
...
- [ ] **Step 1: Demo script green from clean state; rehearse with timer (5–7 min).**
- [ ] **Step 2: Deliverables drafted from real artifacts only; commit.**
- [ ] **Step 3: Novelty check** — review existing OSTIS memory implementations (nika etc.) before any "first/novel" claims.
